...
// ReSharper disable once InconsistentNaming
    ...
    internal sealed class ThrottlerHandle : AbstractTransportAdapterHandle
    {
        ...
        public override bool Write(ByteString payload)
        {
            ...
            {
                var timeOfSend = MonotonicClock.GetElapsed();
...
// ReSharper disable once InconsistentNaming
            ...
            When(ThrottlerState.Throttling, @event =>
            {
                if (@event.FsmEvent is ThrottleMode)
                {
                    ...
                    if(ThrottledMessages.Any())
                        ScheduleDequeue(InboundThrottleMode.TimeToAvailable(MonotonicClock.GetElapsed(), ThrottledMessages.Peek().Length));
                    Sender.Tell(SetThrottleAck.Instance);
                    return Stay();
                }
                ...
                if (@event.FsmEvent is Dequeue)
                {
                    if (ThrottledMessages.Any())
                    {
                        ...
                        InboundThrottleMode = InboundThrottleMode.TryConsumeTokens(MonotonicClock.GetElapsed(),
        ...
        private void ForwardOrDelay(ByteString payload)
        {
            ...
            else
            {
                ...
                {
                    var tokens = payload.Length;
                    var res = InboundThrottleMode.TryConsumeTokens(MonotonicClock.GetElapsed(), tokens);
                    var newBucket = res.Item1;
                    ...
                    else
                    {
                        ...
                        ScheduleDequeue(InboundThrottleMode.TimeToAvailable(MonotonicClock.GetElapsed(), tokens));
                    }
                }
                else
                {
